...
Public Class DGVAllDeliveries
    Private DBConn As Common.DbConnection

    Private Sub DGVAllDeliveries_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            DBConn = New DB2Connection("server=localhost;database=dmpccs5;" + "uid=db2admin;password=db2admin;")
            DBConn.Open()

            With Me.dgvALLDelReceiptRec
                .ColumnCount = 6
            ...
            With Me.dgvAllDelTrans
                .ColumnCount = 5
                .Columns(0).Name = "Delivery Receipt No."
                .Columns(1).Name = "Supplier ID"
                .Columns(2).Name = "Employee  ID"
                .Columns(3).Name = "Transaction Amount"
                .Columns(4).Name = "Date"
                .Columns(0).Width = 80
                .Columns(1).Width = 100
                .Columns(2).Width = 100
    ...
    Private Sub Populate_Grid()
        ...
        Try
            vStr = "SELECT * FROM delv_line_item"
            VCmdPop = New DB2Command(vStr, DBConn)
            VRdrPop = VCmdPop.ExecuteReader
            Me.dgvALLDelReceiptRec.Rows.Clear()
            ...
            While VRdrPop.Read
                ...
                Me.dgvALLDelReceiptRec.Rows.Add(VRow)
            End While

            vStr = "SELECT * FROM deliv_tran"
            VCmdPop = New DB2Command(vStr, DBConn)
            VRdrPop = VCmdPop.ExecuteReader
            Me.dgvAllDelTrans.Rows.Clear()

            While VRdrPop.Read
                VRow = New String() {VRdrPop.GetString(0), VRdrPop.GetString(1), VRdrPop.GetString(2), VRdrPop.GetString(3), VRdrPop.GetString(4)}
                Me.dgvAllDelTrans.Rows.Add(VRow)
            End While
    ...
    Private Sub dgvALLDelReceiptRec_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvALLDelReceiptRec.CellContentClick
